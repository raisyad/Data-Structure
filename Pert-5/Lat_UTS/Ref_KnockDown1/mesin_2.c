...
void printElement(list L) 
{
  ...
  if(L.first != NULL) {
    ...
    while (bantu != NULL) {
      // Proses
      printf("%s %s\n", bantu->kontainer.kb, bantu->kontainer.kb2);
      eKolom* bantu_kol = bantu->col;
      while(bantu_kol != NULL) {
        printf("- %s\n", bantu_kol->kontainer_kol.kk);
...
void sorting(list *L)
{
	...
	do{
				...
				{
                    if(strcmp(prev->kontainer.kb, tunjuk->kontainer.kb)>0)
					{
						tukar = 1;
...
eBaris *getBaris(list L, char label[])
{
  ...
  while(bantu != NULL)
  {
    if(strcmp(bantu->kontainer.kb, label) == 0)
    {
      return bantu;
    }
    bantu = bantu->next;
...
eKolom *getKolom(list L, char label_baris[], char label_kolom[])
{
  ...
  while(bantu1 != NULL)
  {
    if(strcmp(bantu1->kontainer.kb, label_baris) == 0)
    {
		bantu2 = bantu1->col;
		while(bantu2 != NULL)
		{
			if(strcmp(bantu2->kontainer_kol.kk, label_kolom) == 0)
			{
				return bantu2;
			}
...
void printkolom(list L, char label[])
{
	...
	printf("%s %s\n", tempe->kontainer.kb, tempe->kontainer.kb2);
...
void sortingkolom(list *L, char label[])
{
	...
	do{
		...
		{
			if(strcmp(prev_kol->kontainer_kol.kk, tunjuk_kol->kontainer_kol.kk)>0)
			{
				tukar = 1;
...
void sortingkolomall(list *L)
{
	eBaris* tunjuk_row = (*L).first;
		while(tunjuk_row != NULL)
		{
			...
			{
				sortingkolom(L, tunjuk_row->kontainer.kb);
			}
			else
			{
...
void tukarLast(char labelA[], char labelB[], char namaTukar[], list *L)
{
  ...
  else
  {
	...
	{
		if(strcmp(tunjuk->kontainer_kol.kk, namaTukar) == 0 && i == 1)
		{
		...
		else if(strcmp(tunjuk->kontainer_kol.kk, namaTukar) == 0 && i != 1)
		{
...
//   eKolom *firstKolomTokoA = getKolom(*L, labelA, namaTukar1);
  ...
  if(kolomtujuan != NULL)
  {
	...
	{
		// // kondisi kalo isi kolom sama inputan sama
		if(strcmp(tunjuk->kontainer_kol.kk, namaTukar1) == 0 && i == 1)
		{
			barissatu->col = tunjuk->next_kol;
		...
		else if(strcmp(tunjuk->kontainer_kol.kk, namaTukar1) == 0 && i != 1)
		{
		prev->next_kol = tunjuk->next_kol;
		tunjuk->next_kol = kolomtujuan->next_kol;
		kolomtujuan->next_kol = tunjuk;
	...
	while((tunjuk1 != NULL) && (ketemu == 0))
		{
			...
			while ((tunjuk2 != NULL) && (ketemu == 0))
			{
				...
				// printf("%s", tunjuk2->kontainer_kol.kk);
					strcpy(temp, prev2->kontainer_kol.kk);
					// printf(">>%s<<", temp);
				// kondisi kalo si asal ada di kolom ke 1
				if(strcmp(kolomtujuan1->kontainer_kol.kk, barissatu->col->kontainer_kol.kk) == 0)
				{
					// kondisi kalo si tujuan ada di kolom ke 1
					if(strcmp(kolomtujuan2->kontainer_kol.kk, barisdua->col->kontainer_kol.kk) == 0)
					{
						printf("check satu\n");
				...
				else
				{
					...
					else if(strcmp(kolomtujuan2->kontainer_kol.kk, tunjuk2->kontainer_kol.kk) == 0)
					{
						printf("check empat\n");
