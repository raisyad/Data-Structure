  ...
  if(root != NULL){
    ...
    if(hapus != NULL){
      if(hapus->sibling == NULL){
        ...
        if(strcmp(root->child->kontainer.name, del->kontainer.name) == 0){
          delAll(root->child);
          root->child = NULL;
        }
        else{
        ...
        while((hapus->sibling != root->child) && (ketemu == 0)){
          if(strcmp(hapus->kontainer.name, del->kontainer.name) == 0){
              ketemu = 1;
          }
          else{
            prev = hapus;
        ...
        if((ketemu == 0) && (strcmp(hapus->kontainer.name, del->kontainer.name) == 0)){
            ketemu = 1;
        }
...
simpul* findSimpul(data input, simpul *root){
  simpul *hasil = NULL;
  if(root != NULL){
    if(strcmp(root->kontainer.name, input.name) == 0){
      hasil = root;
    }
    else{
...
void printTreePreOrder(simpul *root, int *sumNode){
  if(root != NULL){
    printf("%s %s", root->kontainer.name, root->kontainer.value);
    if (root->kontainer.name && root->kontainer.value && *sumNode != 1)
        printf("-");
    *sumNode = *sumNode - 1;
    simpul *bantu = root->child;
    if(bantu != NULL){
      if(bantu->sibling == NULL){
        /*jika memiliki satu simpul anak*/
        printTreePreOrder(bantu, sumNode);
      }
      else{
        /*jika memiliki banyak simpul anak*/
